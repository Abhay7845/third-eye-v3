...
const YES_NO_ITEMS = [
    ...
    { key: "Increase Façade Height per Floor", label: "Increase Façade Height per Floor" },
    { key: "EHV zone addition - Furniture and interior", label: "EHV zone addition (Furniture & Interior)" },
    { key: "DxC (Equipment and Interiors)", label: "DxC (Equipment and Interiors)" },
...
const DROPDOWN_ITEMS = [
    ...
    { key: "Art & Craft - TYPE 2 & TYPE 3", label: "Art & Craft - TYPE 2 & TYPE 3" },
];
...
function ReadOnlyRow({ label, value, note }) {
    return (
        <tr className="hover:bg-gray-50">
            ...
            <td className="border border-gray-200 px-4 py-3 text-sm text-gray-500 bg-white text-center">—</td>
            <td className="border border-gray-200 px-4 py-3 text-sm font-bold text-right text-gray-800 bg-amber-50">
                ₹ {fmt(value)}
            </td>
            <td className="border border-gray-200 px-4 py-3 text-xs text-gray-400 italic bg-white">{note}</td>
        </tr>
    );
}

function YesNoRow({ label, fieldKey, value, onChange, computedValue, disabled }) {
    return (
        <tr className="hover:bg-gray-50">
            <td className="border border-gray-200 px-4 py-3 text-sm font-semibold text-gray-700 bg-white">
                {label}
            ...
            <td className="border border-gray-200 px-4 py-3 bg-blue-50">
                <select
                    ...
                    disabled={disabled}
                    className={`w-full px-2 py-1.5 border border-blue-200 rounded text-sm focus:outline-none focus:ring-2 focus:ring-blue-400 ${disabled ? "bg-gray-100 cursor-not-allowed text-gray-500" : "bg-white"
                        }`}
                >
...
function DropdownRow({ label, fieldKey, value, onChange, options, computedValue, disabled }) {
    return (
        <tr className="hover:bg-gray-50">
            <td className="border border-gray-200 px-4 py-3 text-sm font-semibold text-gray-700 bg-white">
                {label}
...
export default function Subpage4_1({ handleNext }) {
    ...
    useEffect(() => {
        const fetchCapexRates = async () => {
            try {
                setIsLoading(true);
                const res = await fetch(` http://127.0.0.1:8000/roi_expenses?store_type=${storeData?.store_type}&floor_type=${storeData?.flooring_type}&retail_area=${storeData?.existing_retail_area}`);
                if (res.ok) {
                    const json = await res.json();
                    setDbRates(json.data ?? {});
                    const artOptions = json.data.filter((it) => {
                        if (it.description === 'Art & Craft - TYPE 1' || it.description === 'Art & Craft - TYPE 2 & TYPE 3') {
                            return it
                        }
    ...
    const additionalCapex =
        ...
        (selections.artAndCrafts
            ? computedAmounts.artAndCrafts || 0
            : 0);
    ...
    const handleSave = async () => {
        ...
        try {
            const res = await fetch("http://127.0.0.1:8000/expense_planning_page1", {
                method: "POST",
    ...
    if (isLoading) {
        return (
            <div className="flex items-center justify-center min-h-[300px]">
                <p className="text-gray-500 text-lg animate-pulse">Loading Capex data…</p>
            </div>
        );
    }
    ...
    return (
        <div className="p-6 bg-gradient-to-br from-orange-50 to-amber-50 min-h-screen">
            {/* Header */}
            <div className="mb-8">
                <h2 className="text-3xl font-bold text-gray-800 mb-1">Stage 1 — Capex Planning</h2>
                <p className="text-gray-500 text-sm">
                    Review store-linked data and configure additional capital expenditure items
                </p>
            </div>
            ...
            <div className="bg-white rounded-lg shadow-sm border border-amber-200 p-4 mb-6 grid grid-cols-2 md:grid-cols-4 gap-4">
                {[
                    ...
                    { label: "Carpet Area (sqft)", value: fmt(storeData?.project_type === 'Renovation'? storeData?.new_retail_area : storeData?.existing_retail_area ) },
                ].map(({ label, value }) => (
                ...
                <table className="min-w-full border-collapse">
                    <thead>
                        <tr className="bg-amber-700 text-white text-sm font-semibold">
                            <th className="border border-amber-600 px-4 py-3 text-left min-w-[260px]">Capex Item</th>
                            <th className="border border-amber-600 px-4 py-3 text-center min-w-[160px]">Selection</th>
                            <th className="border border-amber-600 px-4 py-3 text-center min-w-[160px]">Amount (₹)</th>
                            <th className="border border-amber-600 px-4 py-3 text-left min-w-[220px]">Basis</th>
                        </tr>
                    </thead>
                    <tbody>
                        ...
                        <tr className="bg-amber-100 font-bold">
                            <td className="border border-amber-300 px-4 py-3 text-sm text-gray-800">Total Capex</td>
                            <td className="border border-amber-300 px-4 py-3 text-center text-gray-500">—</td>
                        ...
                        <tr className="bg-orange-50 font-bold">
                            <td className="border border-amber-300 px-4 py-3 text-sm text-gray-800">Rate / Sqft</td>
                            <td className="border border-amber-300 px-4 py-3 text-center text-gray-500">—</td>
                        ...
                        <div className="bg-gradient-to-r from-amber-500 to-orange-600 px-8 py-6 rounded-t-2xl">
                            <div className="flex items-center gap-3">
                                ...
                                <div>
                                    ...
                                    <p className="text-orange-100 text-sm mt-0.5">Stage 1 of Expense Planning complete</p>
                                </div>
                            </div>
                        </div>
                        <div className="p-8 space-y-3">
                            <div className="grid grid-cols-2 gap-3">
                                <div className="bg-gray-50 rounded-lg px-4 py-3">
                                    <p className="text-xs text-gray-400 uppercase tracking-wide font-medium">Total Capex</p>
                                    <p className="text-gray-800 font-semibold mt-0.5">₹ {fmt(totalCapex)}</p>
                                </div>
                                <div className="bg-gray-50 rounded-lg px-4 py-3">
                                    <p className="text-xs text-gray-400 uppercase tracking-wide font-medium">Rate / Sqft</p>
                                    <p className="text-gray-800 font-semibold mt-0.5">₹ {fmt(ratePerSqft)}</p>
                                </div>
                            </div>
                        </div>
