                ...
                return (
                    <div key={step.id} className="flex items-center">
                        <div className="flex flex-col items-center">
                            <div
                                className={`w-9 h-9 rounded-full flex items-center justify-center font-bold text-sm border-2 transition-all ${saved
                            ...
                            <span
                                className={`text-xs mt-1 font-medium whitespace-nowrap ${saved
                                    ? "text-green-600"
                                    : active
                                        ? "text-amber-700"
                                        : "text-gray-400"
                        ...
                        {idx < STEPS.length - 1 && (
                            <div
                                className={`h-0.5 w-16 mx-2 mb-5 rounded transition-all ${saved ? "bg-green-400" : "bg-gray-200"
                                    }`}
                            />
                        )}
                    </div>
...
export default function Section4({ roiContext, onNext, onPrevious }) {
    ...
    const handleFetchStoreData = async () => {
        try {
            ...
            const res = await fetch(`http://127.0.0.1:8000/fetchScreen?parameter=roi_store_retail_specifications&roiid=${roiid}`);
            if (res.ok) {
                const json = await res.json();
                setStoreData({ ...json.data[0], 'project_type': roiContext?.projectType });
                setIsFetched(true);
            } else {
                toast.error("Failed to fetch store specifications. Please go back and try again.");
            }
        } catch (err) {
    ...
    return (
        <Section4Context.Provider value={contextValue}>
            ...
            <div className="bg-white shadow-md px-6 py-4 border-b border-gray-200">
                <h1 className="text-2xl font-bold text-gray-800 mb-3">Expense Planning</h1>
                {/* ROI Context Banner */}
                {roiContext?.roiId && (
                    <div className="bg-indigo-50 border border-indigo-200 rounded-xl px-6 py-4">
                        <div className="flex flex-wrap items-center gap-4 text-sm">
                            ...
                            <span className="text-indigo-800 font-bold tracking-wide">{roiContext.roiId}</span>
                            <span className="text-gray-300">|</span>
                            <span className="text-gray-600">Project: <strong className="text-gray-800">{roiContext.projectType}</strong></span>
                            {roiContext.historyId && (
                                <>
                                    <span className="text-gray-300">|</span>
                                    <span className="text-gray-600">History ID: <strong className="text-gray-800">{roiContext.historyId}</strong></span>
                                </>
                            )}
                            {roiContext.refStoreCode && (
                                <>
                                    <span className="text-gray-300">|</span>
                                    <span className="text-gray-600">Reference Store Code: <strong className="text-gray-800">{roiContext.refStoreCode}</strong></span>
                                </>
                            )}
                        </div>
                    </div>
            ...
            {!isFetched ? (
                ...
                <>
                    ...
                    {/* ── Active subpage ───────────────────────────────────── */}
                    {subStep === 1 && (
                        <Subpage4_1 handleNext={handleNext} />
                    )}
                    {subStep === 2 && (
                        <Subpage4_2 handleNext={handleNext} handlePrevious={handlePrevious} />
                    )}
                    {subStep === 3 && (
