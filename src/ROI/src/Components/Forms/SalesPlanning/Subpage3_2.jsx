...
const computeValues = (inputs) => {
    ...
    for (let i = 0; i < 6; i++) {
        totalSales[i] = Math.round((buyersPerDay[i] * avgTicketSize[i] * (parseFloat(inputs.storeDays[0]) || 0)) / 100000);
    }

    const salesGrowthPct = Array(6).fill("0");
    for (let i = 1; i <= 5; i++) {
        const prev = totalSales[i - 1];
        salesGrowthPct[i] = prev > 0
            ? Math.round(((totalSales[i] - prev) / prev) * 100) + "%"
            : "0";
    }
...
function getRefCellClasses(value, refValue) {
    ...
    if (isNaN(num) || isNaN(ref) || num === 0) return { bgColor: "bg-blue-50", textColor: "text-blue-900" };
    if (num > ref) return { bgColor: "bg-green-100", textColor: "text-green-700" };
    if (ref > 0 && num >= ref * 0.95) return { bgColor: "bg-yellow-100", textColor: "text-yellow-700" };
...
function LabelCell({ label, bold = false }) {
    return (
        <td className={`border border-gray-200 px-3 py-2 text-sm text-gray-800 bg-white${bold ? " font-semibold" : ""}`}>
            <strong>{label}</strong>
        </td>
    );
}
    ...
    return (
        <thead>
            <tr className="bg-indigo-700 text-white text-sm font-semibold">
                <th className="border border-indigo-600 px-3 py-2 text-left min-w-[230px]">{label}</th>
                {YEARS.map((yr) => (
                    <th key={yr} className="border border-indigo-600 px-3 py-2 text-center min-w-[95px]">{yr}</th>
                ))}
            </tr>
        </thead>
    );
...
const MIX_FIELDS = new Set([
    "plainShare", "studdedShare", "coinsShare",
    "lcg", "mcg", "hcg", "stoneShareHCG",
    "gis", "regular", "colorStones",
    "solitaireA", "solitaireB", "solitaireC", "solitaireD",
]);
...
export default function Subpage3_2({ handleNext, handlePrevious }) {
    ...
    const { storeParticulars, markStepSaved, setSubpage3_2Data, forwardDetail } = useSection3Context();
    const computed = computeValues(inputs);

    const [validationMetrics, setValidationMetrics] = useState({
        plainShare: 0, studdedShare: 0, coinsShare: 0,
        lcg: 0, mcg: 0, hcg: 0, stoneShareHCG: 0,
        gis: 0, regular: 0, colorStones: 0,
        solitaireA: 0, solitaireB: 0, solitaireC: 0, solitaireD: 0,
    });
        ...
        (async () => {
            try {
                const res = await fetch(
                    `http://127.0.0.1:8000/validation_metrics?region=${encodeURIComponent(region)}&store_format=${encodeURIComponent(fmt)}`
                );
                if (!res.ok) { toast.error("Failed to load validation metrics."); return; }
                ...
                setValidationMetrics({
                    ...
                    solitaireB: d.find((x) => x.Exclusive_Field.includes("Solitaire") && x.Exclusive_Field.includes("B"))?.Region_Value ?? 0,
    ...
    const isFormComplete =
        !hasOver100 && !hasOver100Plain && !hasOver100Studded && !isBelowL1L2Threshold &&
    ...
    if (!(parseFloat(storeParticulars['SBA']) > 0 && parseFloat(storeParticulars['Carpet area']) > 0 &&
        parseFloat(inputs.walkInPerDay[0]) > 0 && parseFloat(inputs.avgTicketSize[0]) > 0 &&
        parseFloat(inputs.storeDays[0]) > 0))
        incompleteReasons.push("Fill all required fields in Sales Planning");
    if (!inputs.increaseWalkIns.slice(1).every((v) => parseFloat(v) >= 0) ||
        !inputs.conversionPct.every((v) => parseFloat(v) >= 0) ||
        !inputs.growthTicketSize.slice(1).every((v) => parseFloat(v) > 0))
    ...
    if (hasOver100Studded)
        ...
        incompleteReasons.push("Studded Sales Mix % must sum to 100% for each year");
    if (isBelowL1L2Threshold)
        incompleteReasons.push("Does Not Fit the minimum threshold of an L1 or L2.5 Store. Please rework the sales projections accordingly");
    ...
    const handleSave = async () => {
        try {
            ...
            const res = await fetch("http://127.0.0.1:8000/sales_planning_page_2", {
                method: "POST",
    ...
    const refInputRow = (label, field, refValue) => (
        <tr key={field}>
            <LabelCell label={label} />
            {YEARS.map((_, i) => {
                const { bgColor, textColor } = getRefCellClasses(inputs[field][i], refValue);
                return (
                    <BlueInputCell
                        key={i}
                        value={inputs[field][i]}
            ...
            <div className="subpage3_2 p-6 bg-gradient-to-br from-blue-50 to-indigo-50 min-h-screen">
                ...
                <div className="space-y-6">
                        ...
                        <table className="min-w-full border-collapse">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    /> */}
                                    {[1, 2, 3, 4, 5, 6].map((i) => <AutoCell key={i} value={storeParticulars['Super Built Up Area']} />)}
                                </tr>
                                ...
                                <tr>
                                    ...
                                    /> */}
                                    {[1, 2, 3, 4, 5, 6].map((i) => <AutoCell key={i} value={storeParticulars['Carpet area']} />)}
                                </tr>
                                ...
                                <tr>
                                    ...
                                    {[1, 2, 3, 4, 5].map((i) => (
                                        <BlueInputCell
                                            ...
                                            onChange={(e) => handleChange("increaseWalkIns", i, e.target.value)}
                                        />
                                    ))}
                                </tr>
                                ...
                                <tr>
                                    ...
                                    <BlueInputCell
                                        ...
                                        onChange={(e) => handleChange("walkInPerDay", 0, e.target.value)}
                                    />
                                    {[1, 2, 3, 4, 5].map((i) => <AutoCell key={i} value={computed.walkInPerDay[i]} />)}
                                </tr>
                                ...
                                <tr>
                                    ...
                                    {computed.buyersPerDay.map((v, i) => <AutoCell key={i} value={v} />)}
                                </tr>
                                ...
                                <tr>
                                    ...
                                    {[1, 2, 3, 4, 5].map((i) => (
                                        <BlueInputCell
                                            ...
                                            onChange={(e) => handleChange("growthTicketSize", i, e.target.value)}
                                        />
                                    ))}
                                </tr>
                                ...
                                <tr>
                                    ...
                                    <BlueInputCell
                                        ...
                                        onChange={(e) => handleChange("avgTicketSize", 0, e.target.value)}
                                    />
                                    {[1, 2, 3, 4, 5].map((i) => <AutoCell key={i} value={computed.avgTicketSize[i]} />)}
                                </tr>
                                ...
                                <tr>
                                    ...
                                    <BlueInputCell
                                        ...
                                        onChange={(e) => handleChange("storeDays", 0, e.target.value)}
                                    />
                                    {[1, 2, 3, 4, 5].map((i) => <AutoCell key={i} value={inputs.storeDays[0]} />)}
                                </tr>
                                ...
                                {isBelowL1L2Threshold && (
                                    <tr>
                                        ...
                                        >
                                            ⚠️ Does Not Fit the minimum threshold of an L1 or L2.5 Store. Please rework the sales projections accordingly
                                        </td>
                                    </tr>
                                )}
                    ...
                    <p className="text-xs text-gray-500 italic px-1">
                        Red indicates significantly less than comparable metrics for the Region and Green indicates significantly higher.
                        Please fill the pricing metrics accurately basis the boutique / cluster / region current numbers.
                    </p>
                    ...
                    <div className="bg-white rounded-lg shadow-md overflow-x-auto">
                        <table className="min-w-full border-collapse">
                            ...
                            <tbody>
                                {refInputRow(`Plain Share - (Ref = ${validationMetrics.plainShare})`, "plainShare", validationMetrics.plainShare)}
                                {refInputRow(`Studded Share - (Ref = ${validationMetrics.studdedShare})`, "studdedShare", validationMetrics.studdedShare)}
                                {refInputRow(`Coins Share - (Ref = ${validationMetrics.coinsShare})`, "coinsShare", validationMetrics.coinsShare)}
                    ...
                    <div className="bg-white rounded-lg shadow-md overflow-x-auto">
                        <table className="min-w-full border-collapse">
                            ...
                            <tbody>
                                {refInputRow(`LCG - (Ref = ${validationMetrics.lcg})`, "lcg", validationMetrics.lcg)}
                                {refInputRow(`MCG - (Ref = ${validationMetrics.mcg})`, "mcg", validationMetrics.mcg)}
                                {refInputRow(`HCG - (Ref = ${validationMetrics.hcg})`, "hcg", validationMetrics.hcg)}
                                {/* Inline warning when any year exceeds 100% */}
                                {hasOver100Plain && (
                                    <tr>
                                        <td
                                ...
                                <TotalRow label="Total Plain Mix" values={computed.totalPlainMix} />
                    ...
                    <div className="bg-white rounded-lg shadow-md overflow-x-auto">
                        <table className="min-w-full border-collapse">
                            ...
                            <tbody>
                                {refInputRow(`GIS - (Ref = ${validationMetrics.gis})`, "gis", validationMetrics.gis)}
                                {refInputRow(`Regular - (Ref = ${validationMetrics.regular})`, "regular", validationMetrics.regular)}
                                {refInputRow(`Color Stones - (Ref = ${validationMetrics.colorStones})`, "colorStones", validationMetrics.colorStones)}
                                {refInputRow(`Solitaire A (<70C) - (Ref = ${validationMetrics.solitaireA})`, "solitaireA", validationMetrics.solitaireA)}
                                {refInputRow(`Solitaire B (70\u2013100C) - (Ref = ${validationMetrics.solitaireB})`, "solitaireB", validationMetrics.solitaireB)}
                                {refInputRow(`Solitaire C (1 CRT+) - (Ref = ${validationMetrics.solitaireC})`, "solitaireC", validationMetrics.solitaireC)}
                                {refInputRow(`Solitaire D (2 CRT+) - (Ref = ${validationMetrics.solitaireD})`, "solitaireD", validationMetrics.solitaireD)}
                                {refInputRow(`Stone Share in Plain (HCG only) - (Ref = ${validationMetrics.stoneShareHCG})`, "stoneShareHCG", validationMetrics.stoneShareHCG)}
                                {/* Inline warning when any year exceeds 100% */}
                                {hasOver100Studded && (
                                    <tr>
                                        <td
                                ...
                                <TotalRow label="Total Studded Mix" values={computed.totalStuddedMix} />
                    ...
                    <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg">
                        <div className="bg-gradient-to-r from-green-500 to-emerald-600 px-8 py-6 rounded-t-2xl">
                            <div className="flex items-center gap-3">
                                ...
                                <div>
                                    <h2 className="text-xl font-bold text-white">Sales Planning Saved</h2>
                                    <p className="text-green-100 text-sm mt-0.5">Step 2 of Sales Planning complete</p>
                                </div>
                            </div>
                        </div>
                        <div className="p-8">
                            <h3 className="text-sm font-semibold text-gray-500 uppercase tracking-wide mb-3">Total Sales (in Lakhs)</h3>
                            <div className="grid grid-cols-3 gap-2">
                                {computed.totalSales.map((v, i) => (
                                    <div key={i} className="bg-gray-50 rounded-lg px-3 py-2">
                                        <p className="text-xs text-gray-400 font-medium">Yr. {i + 1}</p>
                                        <p className="text-gray-800 font-semibold mt-0.5">{v?.toLocaleString("en-IN") ?? "—"}</p>
                                    </div>
                                ))}
                            </div>
                        </div>
