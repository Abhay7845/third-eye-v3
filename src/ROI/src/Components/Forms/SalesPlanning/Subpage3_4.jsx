...
function computeValues(subpage3_2Data, customerDiscount) {
    ...
    for (let i = 0; i < 6; i++) {
        ...
        // Customer Discount
        const cdLCGVal = totalSales * plainShare * lcg * (customerDiscount["LCG"] ?? 0);
        const cdMCGVal = totalSales * plainShare * mcg * (customerDiscount["MCG"] ?? 0);
        const cdHCGVal = totalSales * plainShare * hcg * (customerDiscount["HCG"] ?? 0);

        const cdGISVal = totalSales * studdedShare * gis * (customerDiscount["GIS"] ?? 0);
        const cdRegularVal = totalSales * studdedShare * regular * (customerDiscount["Regular"] ?? 0);
        const cdColorStonesVal = totalSales * studdedShare * colorStones * (customerDiscount["Color Stones"] ?? 0);

        const cdSolitaireAVal = totalSales * studdedShare * solitaireA * (customerDiscount["Solitaire A(<70C)"] ?? 0);
        const cdSolitaireBVal = totalSales * studdedShare * solitaireB * (customerDiscount["Solitaire B(70-100C)"] ?? 0);
        const cdSolitaireCVal = totalSales * studdedShare * solitaireC * (customerDiscount["Solitaire C(1CRT+)"] ?? 0);
        const cdSolitaireDVal = totalSales * studdedShare * solitaireD * (customerDiscount["Solitaire D(2CRT+)"] ?? 0);

        const cdCoinsVal = totalSales * coinsShare * (customerDiscount["Coins"] ?? 0);

        const cdTotalVal =
            cdLCGVal + cdMCGVal + cdHCGVal +
            cdGISVal + cdRegularVal + cdColorStonesVal +
            cdSolitaireAVal + cdSolitaireBVal +
            cdSolitaireCVal + cdSolitaireDVal +
            cdCoinsVal;
    ...
    return {
        cdLCG, cdMCG, cdHCG, cdGIS, cdRegular, cdColorStones,
        cdSolitaireA, cdSolitaireB, cdSolitaireC, cdSolitaireD,
        cdCoins, cdTotal, cdPctOfUCP,
        ghsPlain, ghsStudded, ghsCoins, ghsTotal, ghsPctOfUCP
    };
}
...
function SubSectionRow({ label }) {
    return (
        <tr className="bg-blue-100">
            <td colSpan={7} className="border border-blue-200 px-3 py-1.5 text-sm font-bold text-blue-800">
                {label}
            </td>
        </tr>
    );
...
export default function Subpage3_4({ handleNext, handlePrevious }) {
    ...
    console.log('Page 3_4', subpage3_2Data)
    const getCustomerDiscountData = async () => {
        try {
            setLoading(true);
            const res = await fetch(`http://127.0.0.1:8000/cutomer_discount`);

            if (res.ok) {
                const response = await res.json();
                const data = response.data;
                const discountObj = data?.reduce((acc, curr) => ({
                    ...acc,
                    ...curr
                }
                ), {})
    ...
    const handleSave = async () => {
        try {
            ...
            const res = await fetch("http://127.0.0.1:8000/sales_planning_page_4", {
                method: "POST",
    ...
    return (
        <div>
            {
                isLoading ? <h1>Calculating the Data for The Customer Discount</h1>
                    :
                    <>
                                ...
                                <div className="bg-white rounded-lg shadow-md overflow-x-auto">
                                    <table className="min-w-full border-collapse">
                                        ...
                                        <tbody>
                                            ...
                                            <tr>
                                                <LabelCell label="LCG" />
                                                {computed.cdLCG.map((v, i) => <AutoCell key={i} value={v} />)}
                                            </tr>
                                            <tr>
                                                <LabelCell label="MCG" />
                                                {computed.cdMCG.map((v, i) => <AutoCell key={i} value={v} />)}
                                            </tr>
                                            <tr>
                                                <LabelCell label="HCG" />
                                                {computed.cdHCG.map((v, i) => <AutoCell key={i} value={v} />)}
                                            </tr>
                                            ...
                                            <tr>
                                                <LabelCell label="GIS" />
                                                {computed.cdGIS.map((v, i) => <AutoCell key={i} value={v} />)}
                                            </tr>
                                            <tr>
                                                <LabelCell label="Regular" />
                                                {computed.cdRegular.map((v, i) => <AutoCell key={i} value={v} />)}
                                            </tr>
                                            <tr>
                                                <LabelCell label="Color Stones" />
                                                {computed.cdColorStones.map((v, i) => <AutoCell key={i} value={v} />)}
                                            </tr>
                                            <tr>
                                                <LabelCell label="Solitaire A (<70C)" />
                                                {computed.cdSolitaireA.map((v, i) => <AutoCell key={i} value={v} />)}
                                            </tr>
                                            <tr>
                                                <LabelCell label="Solitaire B (70–100C)" />
                                                {computed.cdSolitaireB.map((v, i) => <AutoCell key={i} value={v} />)}
                                            </tr>
                                            <tr>
                                                <LabelCell label="Solitaire C (1 CRT+)" />
                                                {computed.cdSolitaireC.map((v, i) => <AutoCell key={i} value={v} />)}
                                            </tr>
                                            <tr>
                                                <LabelCell label="Solitaire D (2 CRT+)" />
                                                {computed.cdSolitaireD.map((v, i) => <AutoCell key={i} value={v} />)}
                                            </tr>
                                            ...
                                            <tr>
                                                ...
                                                {computed.cdCoins.map((v, i) => <AutoCell key={i} value={v} />)}
                                            </tr>
                                    ...
                                    <table className="min-w-full border-collapse">
                                        ...
                                        <tbody>
                                            <tr>
                                                ...
                                                {computed.ghsPlain.map((v, i) => <AutoCell key={i} value={v} />)}
                                            </tr>
                                            <tr>
                                                <LabelCell label="Studded" />
                                                {computed.ghsStudded.map((v, i) => <AutoCell key={i} value={v} />)}
                                            </tr>
                                            <tr>
                                                <LabelCell label="Coins" />
                                                {computed.ghsCoins.map((v, i) => <AutoCell key={i} value={v} />)}
                                            </tr>
                    ...
                    <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg">
                        <div className="bg-gradient-to-r from-green-500 to-emerald-600 px-8 py-6 rounded-t-2xl">
                            <div className="flex items-center gap-3">
                                ...
                                <div>
                                    <h2 className="text-xl font-bold text-white">Discounts Saved</h2>
                                    <p className="text-green-100 text-sm mt-0.5">Sales Planning complete</p>
                                </div>
                            </div>
                        </div>
                        <div className="p-8">
                            <h3 className="text-sm font-semibold text-gray-500 uppercase tracking-wide mb-3">Total Customer Discount (Yr. 1–6)</h3>
