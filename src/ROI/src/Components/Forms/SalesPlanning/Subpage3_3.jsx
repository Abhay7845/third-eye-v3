...
const computeValues = (inputs, subpage3_2Data) => {
    ...
    bgTotalStockTurn[0] = (Number(bgCoinsStockTurn[0]) + Number(bgPlainStockTurn[0]) + Number(bgStuddedStockTurn[0])).toFixed(2)
    for (let i = 0; i < 5; i++) {
        ...
        bgTotalStockTurn[i + 1] = (Number(bgCoinsStockTurn[i + 1]) + Number(bgPlainStockTurn[i + 1]) + Number(bgStuddedStockTurn[i + 1])).toFixed(2)
    }
    return {
        totalStockTurn,
        remainingStockTurn,
...
export default function Subpage3_3({ handleNext, handlePrevious }) {
    ...
    if (!(parseFloat(inputs.lcgAMC[0]) > 0 && parseFloat(inputs.mcgAMC[0]) > 0 && parseFloat(inputs.hcgAMC[0]) > 0))
    ...
    if (!inputs.stockTurnPlain.every((v) => parseFloat(v) > 0) ||
        !inputs.stockTurnStudded.every((v) => parseFloat(v) > 0) ||
        !inputs.stockTurnCoins.every((v) => parseFloat(v) > 0))
        incompleteReasons.push("Fill all Stock Turn values for all 6 years");
    if (!inputs.bgCoinsStockTurn[0] > 0)
        incompleteReasons.push("Fill Brand Guidelines Coins Stock Turn for all 6 years");
    ...
    const handleSave = async () => {
        try {
            ...
            const res = await fetch("http://127.0.0.1:8000/sales_planning_page_3", {
                method: "POST",
    ...
    const MIX_FIELDS = new Set([
        "stockTurnPlain", "stockTurnStudded", "stockTurnCoins", "baseRate22K", "markupPct", "lcgAMC", "mcgAMC", "hcgAMC", "coinsAMC"
    ]);
    ...
    const fetchStockTurnGuideLine = async (parameter) => {
        ...
        try {
            const response = await fetch("http://127.0.0.1:8000/stock_turn_guideline", {
                method: "POST",
                headers: {
            ...
            <div className="subpage3_3 p-6 bg-gradient-to-br from-blue-50 to-indigo-50 min-h-screen">
                ...
                <div className="space-y-6">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <BlueInputCell
                                        ...
                                        onChange={(e) => handleChange("baseRate22K", 0, e.target.value)}
                                    />
                                    {[1, 2, 3, 4, 5].map((i) => (
                                ...
                                <tr>
                                    ...
                                    <BlueInputCell
                                        ...
                                        onChange={(e) => handleChange("markupPct", 0, e.target.value)}
                                    />
                                    {[1, 2, 3, 4, 5].map((i) => (
                                ...
                                <tr>
                                    ...
                                    <BlueInputCell
                                        ...
                                        onChange={(e) => handleChange("coinsAMC", 0, e.target.value)}
                                    />
                                    {[1, 2, 3, 4, 5].map((i) => (
                    ...
                    <div className="bg-yellow-50 border-l-4 border-yellow-400 px-4 py-3 rounded text-xs text-yellow-800">
                        For L1 / L2 / L4 inventory values are calculated on brand guidelines and will not change basis the below stock turns.
                        For L3 / L2.5 Formats please modify the same if required considering Inventory requirements and partner's ROI.
                    </div>
                    ...
                    <div className="bg-white rounded-lg shadow-md overflow-x-auto">
                        <table className="min-w-full border-collapse">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    <LabelCell label="Plain" />
                                    {computed.bgPlainStockTurn.map((v, i) => <AutoCell key={i} value={v} />)}
                                </tr>

                                {/* Studded — all auto (from DB) */}
                                <tr>
                                    <LabelCell label="Studded" />
                                    {computed.bgStuddedStockTurn.map((v, i) => <AutoCell key={i} value={v} />)}
                                </tr>
                                ...
                                <tr>
                                    ...
                                    <BlueInputCell
                                        ...
                                        onChange={(e) => handleChange("bgCoinsStockTurn", 0, e.target.value)}
                                    />
                    ...
                    <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg">
                        <div className="bg-gradient-to-r from-green-500 to-emerald-600 px-8 py-6 rounded-t-2xl">
                            <div className="flex items-center gap-3">
                                ...
                                <div>
                                    <h2 className="text-xl font-bold text-white">Pricing Metrics Saved</h2>
                                    <p className="text-green-100 text-sm mt-0.5">Step 3 of Sales Planning complete</p>
                                </div>
                            </div>
                        </div>
                        <div className="p-8">
                            <h3 className="text-sm font-semibold text-gray-500 uppercase tracking-wide mb-3">Total Stock Value</h3>
