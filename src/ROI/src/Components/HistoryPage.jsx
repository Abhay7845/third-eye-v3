...
const PAGES = [
  ...
  { name: "Store Retail Specifications",                 icon: "📋", step: 2, group: "Store Setup" },
  { name: "Sales Planning - Ref Store Code Details",     icon: "🔗", step: 3, group: "Sales Planning" },
  { name: "Sales Planning - Sales Summary",              icon: "📈", step: 3, group: "Sales Planning" },
  { name: "Sales Planning - Stock Summary",              icon: "📦", step: 3, group: "Sales Planning" },
  { name: "Sales Planning - Discount",                   icon: "🏷️", step: 3, group: "Sales Planning" },
...
function getFirstIncompleteStep(pages) {
  ...
  const m = Object.fromEntries(pages.map(p => [p.page_name ?? p.PageName, p.status]));
  const na = name => !m[name] || m[name] === "Not Available";
  if (na("Basic Store Details")) return 1;
  if (na("Store Retail Specifications")) return 2;
  if (["Sales Planning - Ref Store Code Details", "Sales Planning - Sales Summary",
       "Sales Planning - Stock Summary", "Sales Planning - Discount"].some(na)) return 3;
  if (["Capex Expenses", "Resource Expenses", "Other Expenses", "Summary Expenses"].some(na)) return 4;
  return 5;
}

function getStats(pages) {
  if (!pages?.length) return { done: 0, total: 10, pct: 0 };
  const done = pages.filter(p => p.status === "Pending" || p.status === "Submitted to RBM").length;
  return { done, total: pages.length, pct: Math.round((done / pages.length) * 100) };
}
...
export default function HistoryPage({ onBack, onContinueROI }) {
  ...
  const [viewModal, setViewModal] = useState({ open: false, pageName: "", data: null, loading: false });

  useEffect(() => {
    (async () => {
      try {
        setListLoading(true);
        const res = await fetch(`${BASE}/roi_id`);
  ...
  const handleContinue = async () => {
    ...
    try {
      const [r1, r2] = await Promise.all([
        fetch(`${BASE}/fetchScreen?parameter=roi_basic_store_details&roiid=${selectedRoi.roiid}`),
        fetch(`${BASE}/fetchScreen?parameter=roi_store_retail_specifications&roiid=${selectedRoi.roiid}`),
      ]);
      ...
      const existingStoreCode = d1.exsisting_store_code ?? d1.existing_store_code ?? "";
  ...
  const handleViewPage = async (pageName) => {
    ...
    try {
      ...
      const sp = (screen) => fetch(`${BASE}/sales_planning`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ screen, roiid }),
      });
      const fs = (param) => fetch(`${BASE}/fetchScreen?parameter=${param}&roiid=${roiid}`);

      let res;
      switch (pageName) {
        case "Basic Store Details":                     res = await fs("roi_basic_store_details"); break;
        case "Store Retail Specifications":             res = await fs("roi_store_retail_specifications"); break;
        case "Sales Planning - Ref Store Code Details": res = await sp(1); break;
        case "Sales Planning - Sales Summary":          res = await sp(2); break;
        case "Sales Planning - Stock Summary":          res = await sp(3); break;
        case "Sales Planning - Discount":               res = await sp(4); break;
        case "Capex Expenses":                          res = await fetch(`${BASE}/expense_details/${roiid}?expense_type=CAPEX`); break;
        case "Resource Expenses":                       res = await fetch(`${BASE}/expense_details/${roiid}?expense_type=RESOURCE`); break;
        case "Other Expenses":                          res = await fetch(`${BASE}/expense_details/${roiid}?expense_type=OTHER`); break;
        case "Summary Expenses":                        res = await fetch(`${BASE}/expense_details/${roiid}?expense_type=SUMMARY`); break;
        default: res = null;
      }

      if (res?.ok) {
        const json = await res.json();
        setViewModal(prev => ({ ...prev, data: json.data ?? [], loading: false }));
      } else {
  ...
  // ── Derived filter data ─────────────────────────────────────────────────
  const projectTypes = ["All", ...new Set(
  ...
  const filteredList = roiList.filter(roi => {
    ...
    const roiDate  = roi.created_date ?? roi.create_date ?? roi.inserted_date ?? roi.date ?? "";
    return (
      (q === "" || id.toLowerCase().includes(q) || projType.toLowerCase().includes(q)) &&
  ...
  const groupedPages = summary
    ? GROUPS.map(group => ({
        ...
        pages: PAGES
          ...
          .map(p => {
            const match = summary.find(s => (s.page_name ?? s.PageName) === p.name);
            return {
              ...p,
        ...
        <aside className="w-72 flex-shrink-0 flex flex-col border-r border-gray-200 bg-white overflow-hidden">
          ...
          <div className="flex-shrink-0 p-3 border-b border-gray-100 space-y-2">
            ...
            <div className="grid grid-cols-2 gap-1">
              <div>
                <p className="text-[9px] uppercase text-gray-400 font-semibold mb-0.5">From</p>
                <input
                  type="date"
                  value={dateFrom}
              ...
              <div>
                <p className="text-[9px] uppercase text-gray-400 font-semibold mb-0.5">To</p>
                <input
                  type="date"
                  value={dateTo}
          ...
          <div className="flex-1 overflow-y-auto p-2 space-y-1.5">
            {listLoading ? (
              ...
              filteredList.map((roi, idx) => {
                ...
                const projType = roi.project_type ?? roi.projectType ?? "—";
                const colorCls = PROJECT_COLORS[projType] ?? "bg-gray-100 text-gray-500 border-gray-200";
                ...
                return (
                  ...
                  >
                    <div className="flex items-center justify-between gap-2">
                      <span className="text-xs font-bold text-gray-800 font-mono truncate">{id}</span>
                    ...
                    <div className="flex items-center justify-between mt-1.5 gap-1">
                      ...
                      {roi.inserted_date && (
                        <span className="text-[10px] text-indigo-400 font-medium">{daysAgo(roi.inserted_date)}</span>
                      )}
                    </div>
                  </button>
                );
        ...
        <main className="flex-1 overflow-y-auto p-8">
          ...
          {!selectedRoi && !summaryLoading && (
            <div className="flex flex-col items-center justify-center h-full text-center">
              ...
              <p className="text-lg font-semibold text-gray-500">Select an ROI request</p>
              <p className="text-sm text-gray-400 mt-1">Click any request on the left to view its progress</p>
            </div>
          )}
          ...
          {summaryLoading && (
            <div className="flex items-center justify-center h-full">
              <div className="flex flex-col items-center gap-3">
                ...
                <span className="text-sm font-medium text-gray-500">Loading summary…</span>
              </div>
            </div>
          )}
            ...
            <div className="max-w-2xl mx-auto space-y-5">
              ...
              <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
                <div className="flex items-start justify-between gap-4">
                  <div>
                    <p className="text-[10px] uppercase tracking-widest text-gray-400 font-semibold mb-1">ROI ID</p>
                    <p className="text-2xl font-extrabold text-gray-900 font-mono tracking-wide">
                      {selectedRoi.roiid}
                    </p>
                    <span className={`inline-block text-xs font-semibold px-2.5 py-1 rounded-full border mt-2 ${
                      PROJECT_COLORS[selectedRoi.project_type ?? selectedRoi.projectType]
                      ?? "bg-gray-100 text-gray-500 border-gray-200"
                    }`}>
                  ...
                  <div className="flex flex-col items-center gap-1 flex-shrink-0">
                    <div className="relative w-20 h-20">
                      ...
                      <div className="absolute inset-0 flex items-center justify-center">
                        <span className="text-sm font-bold text-gray-700">{stats.pct}%</span>
                      </div>
                    </div>
                    <p className="text-xs text-gray-400">{stats.done}/{stats.total} pages</p>
                  </div>
                </div>
              ...
              {groupedPages.map(({ group, pages }) => {
                const groupDone = pages.filter(p => p.status === "Pending").length;
                ...
                return (
                  <div key={group} className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
                    <div className="px-6 py-3.5 border-b border-gray-50 flex items-center justify-between">
                      <div className="flex items-center gap-2">
                        <span className={`w-2 h-2 rounded-full ${
                          allDone ? "bg-green-500" : noneDone ? "bg-gray-300" : "bg-amber-400"
                        }`} />
                        <h3 className="font-bold text-sm text-gray-700">{group}</h3>
                      </div>
                    ...
                    <div className="divide-y divide-gray-50">
                      {pages.map((page) => {
                          const done = page.status === "Pending" || page.status === "Submitted to RBM";
                          const pageSubmitted = page.status === "Submitted to RBM";
                        return (
                          <div key={page.name} className="flex items-center gap-3 px-6 py-3">
                            <span className="text-lg flex-shrink-0">{page.icon}</span>
                            <div className="flex-1 min-w-0">
                              <p className={`text-sm font-medium truncate ${done ? "text-gray-800" : "text-gray-400"}`}>
                                {page.name}
                              </p>
                              {done && page.inserted_date && (
                                <p className="text-[10px] text-gray-400 mt-0.5">
                                  Saved: {page.inserted_date}
                                  {page.updated_date ? ` · Updated: ${page.updated_date}` : ""}
                                </p>
                              )}
                            </div>
              ...
              {isSubmitted ? (
                <div className="rounded-2xl border border-green-200 bg-green-50 p-6 flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-green-600 mt-1">This ROI has been submitted for approval. All sections are view-only.</p>
                  </div>
                </div>
              ) : (
                <div className={`rounded-2xl border p-6 flex items-center justify-between gap-4 ${
                  isAllComplete ? "bg-green-50 border-green-200" : "bg-indigo-50 border-indigo-200"
                }`}>
                  <div>
                    <p className="font-bold text-gray-800">
                      {isAllComplete ? "🎉 All sections complete" : `📍 Continue from Step ${firstIncompleteStep}`}
                    </p>
                  ...
                  >
                    {continuing ? "Loading…" : isAllComplete ? "Review & Submit" : "Continue ROI →"}
                  </button>
                </div>
              )}
        ...
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[80vh] flex flex-col">
            ...
            <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100 flex-shrink-0">
              <div>
                <h2 className="text-lg font-bold text-gray-900">{viewModal.pageName}</h2>
                <p className="text-xs text-gray-400 mt-0.5">ROI ID: {selectedRoi?.roiid}</p>
              </div>
              <button
                        ...
                        <table className="min-w-full border-collapse text-xs">
                          <thead>
                            <tr className="bg-indigo-700 text-white">
                              <th className="px-3 py-2 text-left font-semibold min-w-[200px]">Metric</th>
                          ...
                          <tbody>
                            {viewModal.data.map((row, i) => (
                              <tr key={i} className={i % 2 === 0 ? "bg-white" : "bg-gray-50"}>
                                <td className="px-3 py-2 font-medium text-gray-700 border-b border-gray-100">{row.Header}</td>
                                {["Yr1","Yr2","Yr3","Yr4","Yr5","Yr6"].map(y => (
                                  <td key={y} className="px-3 py-2 text-right text-gray-600 border-b border-gray-100 tabular-nums">
                                    {row[y] !== null && row[y] !== undefined ? row[y] : "—"}
                                  </td>
                                ))}
                              </tr>
                            ))}
                  ...
                  return (
                    <div className="space-y-3">
                      {viewModal.data.map((row, rowIdx) => {
                        ...
                        return (
                          <div key={rowIdx} className="bg-gray-50 rounded-xl p-4">
                            {viewModal.data.length > 1 && (
                              <p className="text-[10px] text-gray-400 font-semibold uppercase tracking-wide mb-3">Record {rowIdx + 1}</p>
                            )}
